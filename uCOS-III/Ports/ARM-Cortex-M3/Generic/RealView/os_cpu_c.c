...
{
    CPU_SR_ALLOC();                                         //分配保存中断状态的局部变量，后面关闭中断可以保存中断状态


    CPU_CRITICAL_ENTER();                                   //与CPU_CRITICAL_EXIT() 之间形成临界段，避免期间程序程序运行的时候受到干扰
    OSIntNestingCtr++;                                      //进入中断时中断嵌套数要+1
    CPU_CRITICAL_EXIT();

    OSTimeTick();                                           

    OSIntExit();                                            /* 退出中断，里面中断数嵌套数-1            */
}
...
void  OS_CPU_SysTickInit (CPU_INT32U  cnts)
{
    ...
    //填入Systick重载记数值
    CPU_REG_NVIC_ST_RELOAD = cnts - 1u;                     //Systick以该值为计数值循环记数

    //  设置Systick中断优先级                                                      
    prio  = CPU_REG_NVIC_SHPRI3;
    prio &= DEF_BIT_FIELD(24, 0);
    prio |= DEF_BIT_MASK(OS_CPU_CFG_SYSTICK_PRIO, 24);      //设置默认最高优先级为24，裸机恰恰相反为最低优先级

    CPU_REG_NVIC_SHPRI3 = prio;

    //使能Systick 的时钟源 和启动计数器                                                                                              
