...
void          OSMutexCreate             (OS_MUTEX              *p_mutex,
                                         CPU_CHAR              *p_name,
                                         OS_ERR                *p_err);         //创建一个互斥信号量

#if OS_CFG_MUTEX_DEL_EN > 0u
OS_OBJ_QTY    OSMutexDel                (OS_MUTEX              *p_mutex,
                                         OS_OPT                 opt,    
                                         OS_ERR                *p_err);         //删除一个互斥信号量
#endif
...
void          OSMutexPend               (OS_MUTEX              *p_mutex,
                                         ...
                                         OS_OPT                 opt,
                                         CPU_TS                *p_ts,
                                         OS_ERR                *p_err);         //等待一个互斥信号量

#if OS_CFG_MUTEX_PEND_ABORT_EN > 0u
OS_OBJ_QTY    OSMutexPendAbort          (OS_MUTEX              *p_mutex,        
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);         //取消一个任务对一个互斥信号量的等待
#endif

void          OSMutexPost               (OS_MUTEX              *p_mutex,        //发布一个互斥信号量
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);
...
void          OSQCreate                 (OS_Q                  *p_q,
                                         CPU_CHAR              *p_name,
                                         OS_MSG_QTY             max_qty,
                                         OS_ERR                *p_err);     //创建一个消息队列

#if OS_CFG_Q_DEL_EN > 0u
OS_OBJ_QTY    OSQDel                    (OS_Q                  *p_q,
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);     //删除一个消息队列
#endif
...
void          OSSemCreate               (OS_SEM                *p_sem,
                                         CPU_CHAR              *p_name,
                                         OS_SEM_CTR             cnt,
                                         OS_ERR                *p_err);        //创建一个多值信号量

OS_OBJ_QTY    OSSemDel                  (OS_SEM                *p_sem,
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);         //删除一个多值信号量

OS_SEM_CTR    OSSemPend                 (OS_SEM                *p_sem,          
                                         OS_TICK                timeout,
                                         OS_OPT                 opt,
                                         CPU_TS                *p_ts,
                                         OS_ERR                *p_err);         //等待一个多值信号量

#if OS_CFG_SEM_PEND_ABORT_EN > 0u
OS_OBJ_QTY    OSSemPendAbort            (OS_SEM                *p_sem,          //中止任务对多值信号量的等待
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);
#endif

OS_SEM_CTR    OSSemPost                 (OS_SEM                *p_sem,              //发布多值信号量
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);

#if OS_CFG_SEM_SET_EN > 0u
void          OSSemSet                  (OS_SEM                *p_sem,          //设置多值信号量的记数值
                                         OS_SEM_CTR             cnt,
                                         OS_ERR                *p_err);
#endif
...
void          OSTimeDly                 (OS_TICK                dly,                //延迟函数(单位：时钟节拍)
                                         OS_OPT                 opt,
                                         OS_ERR                *p_err);

#if OS_CFG_TIME_DLY_HMSM_EN > 0u
void          OSTimeDlyHMSM             (CPU_INT16U             hours,              //延迟函数（单位：时、分、秒、毫秒）
                                         CPU_INT16U             minutes,
                                         CPU_INT16U             seconds,
                                         CPU_INT32U             milli,
                                         OS_OPT                 opt,
...
void          OSTimeDlyResume           (OS_TCB                *p_tcb,              //取消函数的延时（直接进入就绪态）
                                         OS_ERR                *p_err);
#endif

OS_TICK       OSTimeGet                 (OS_ERR                *p_err);             //获取当前时钟节拍
    
void          OSTimeSet                 (OS_TICK                ticks,              //时钟节拍设置（谨慎使用）
                                         OS_ERR                *p_err);

void          OSTimeTick                (void);                                     //时钟节拍服务函数（用户无需关心）
...
#if OS_CFG_TMR_EN > 0u
void          OSTmrCreate               (OS_TMR                *p_tmr,                 //创建一个软件定时器
                                         CPU_CHAR              *p_name,
                                         OS_TICK                dly,
                                         OS_TICK                period,
                                         OS_OPT                 opt,
...
CPU_BOOLEAN   OSTmrDel                  (OS_TMR                *p_tmr,                  //删除一个软件定时器
                                         OS_ERR                *p_err);

OS_TICK       OSTmrRemainGet            (OS_TMR                *p_tmr,                  //获得定时器定时剩余节拍
                                         OS_ERR                *p_err);

CPU_BOOLEAN   OSTmrStart                (OS_TMR                *p_tmr,                  //定时器开始记数（如果是停止的也是重新记数）
                                         OS_ERR                *p_err);

OS_STATE      OSTmrStateGet             (OS_TMR                *p_tmr,                  //获得定时器状态
                                         OS_ERR                *p_err);

CPU_BOOLEAN   OSTmrStop                 (OS_TMR                *p_tmr,                   //定时器暂停
                                         OS_OPT                 opt,
                                         void                  *p_callback_arg,
                                         OS_ERR                *p_err);
