...
void  OS_PendListInit (OS_PEND_LIST  *p_pend_list)      //复位等待列表的所有成员
{
...
{
    switch (p_tcb->TaskState) {
        case OS_TASK_STATE_RDY:                                  /* 任务就绪态           */
        case OS_TASK_STATE_DLY:                                  /* 任务延迟态         */
        case OS_TASK_STATE_SUSPENDED:                            /* 任务挂起态                  */
        case OS_TASK_STATE_DLY_SUSPENDED:                        /* 任务延迟中被挂起态 */
             break;

        case OS_TASK_STATE_PEND:                                //无期限等待态
        case OS_TASK_STATE_PEND_TIMEOUT:                        //有期限等待态
...
#if (OS_MSG_EN > 0u)                //如果使能任务队列或消息队列
                 p_tcb->MsgPtr  = p_void;                        /* 保存信息到等待任务        */
             ...
             p_tcb->PendStatus = OS_STATUS_PEND_OK;              /* 清楚等待状态                                 */
             p_tcb->PendOn     = OS_TASK_PEND_ON_NOTHING;        /* 标记不在等待                       */
             break;

        case OS_TASK_STATE_PEND_SUSPENDED:                      //无期限等待挂起态
        case OS_TASK_STATE_PEND_TIMEOUT_SUSPENDED:              //有限期限等待挂起态
             if (p_tcb->PendOn == OS_TASK_PEND_ON_MULTI) {          //如果任务在等待多个信息量或消息队列
                 OS_Post1(p_obj,                                 /* 标记哪个内核对象被发布               */
                          p_tcb,
                          p_void,
                          msg_size,
                          ts);
             } else {                       //如果任务不在等待多个信息量或消息队列
#if (OS_MSG_EN > 0u)                        //如果使能调试代码和变量
                 p_tcb->MsgPtr  = p_void;                        /* 保存信息到等待任务        */
                 p_tcb->MsgSize = msg_size;                      /*                  */
#endif
                 p_tcb->TS      = ts;       //保存时间戳到等待任务
             ...
             p_tcb->TaskState  = OS_TASK_STATE_SUSPENDED;           //任务改为挂起态
