...
*
* Arguments  : p_q         is a pointer to the message queue         一个消息队列的指针
*
*              p_name      is a pointer to an ASCII string that will be used to name the message queue  消息队列的值
*
*              max_qty     indicates the maximum size of the message queue (must be non-zero).  Note that it's also not 最大信息数
...
*
*                            OS_OPT_DEL_NO_PEND          Delete the queue ONLY if no task pending                  如果没有等待任务删除消息队列
*                            OS_OPT_DEL_ALWAYS           Deletes the queue even if tasks are waiting.              必须删除消息队列
...
*                            OS_ERR_TASK_WAITING         One or more tasks were waiting on the queue                还有任务在执行
*
...
*                            arrives. 等待超时时间，0为无期限等待
*
*              opt           determines whether the user wants to block if the queue is empty or not:
*
*                                OS_OPT_PEND_BLOCKING       堵塞当前任务直到获得信息
*                                OS_OPT_PEND_NON_BLOCKING   不堵塞当前任务，不继续等待信息
*
*              p_msg_size    is a pointer to a variable that will receive the size of the message     信息大小（单位：字节）
*
...
*                            is valid and indicates that you don't need the timestamp.      返回的时间戳
*
*              p_err         is a pointer to a variable that will contain an error code returned by this function.
*
...
*
*                            OS_OPT_PEND_ABORT_1          ABORT wait for a single task (HPT) waiting on the queue       只中止该消息队列等待列表最高优先级的任务
*                            OS_OPT_PEND_ABORT_ALL        ABORT wait for ALL tasks that are  waiting on the queue       中止该消息队列等待列表所有任务
*                            OS_OPT_POST_NO_SCHED         Do not call the scheduler                                     不进行消息调度
*
*              p_err     is a pointer to a variable that will contain an error code returned by this function.
*
...
*
*              p_void        is a pointer to the message to send.           消息指针
*
*              msg_size      specifies the size of the message (in bytes)   消息大小（单位：字节）
*
*              opt           determines the type of POST performed:
*
*                                OS_OPT_POST_ALL          POST to ALL tasks that are waiting on the queue.  This option     唤醒所有等待任务
*                                                         can be added to either OS_OPT_POST_FIFO or OS_OPT_POST_LIFO
*                                OS_OPT_POST_FIFO         POST message to end of queue (FIFO) and wake up a single      发布到队列的入口端
*                                                         waiting task.
*                                OS_OPT_POST_LIFO         POST message to the front of the queue (LIFO) and wake up     发布到队列的出口端
*                                                         a single waiting task.
*                                OS_OPT_POST_NO_SCHED     Do not call the scheduler                                     不进行任务调度
*
...
*                                        OS_OPT_POST_FIFO + OS_OPT_POST_ALL + OS_OPT_POST_NO_SCHED                  发布到队列的入口端，唤醒所有等待任务，不进行任务调度（以此类推）
