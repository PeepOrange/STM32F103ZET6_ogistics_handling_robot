...
*
* Arguments  : timeout       is the amount of time you are will to wait for the signal      等待超时时间，为0的话无期限等待
*
*              opt           determines whether the user wants to block if a semaphore post was not received:
*
*                                OS_OPT_PEND_BLOCKING           堵塞
*                                OS_OPT_PEND_NON_BLOCKING       不堵塞
*
...
*              opt       provides options for this function:
*
*                            OS_OPT_POST_NONE         No option selected        没有操作
*                            OS_OPT_POST_NO_SCHED     Indicates that the scheduler will not be called.      不进行任务调度
*
*              p_err     is a pointer to a variable that will contain an error code returned by this function.
*
*                            OS_ERR_NONE              If the task was readied and informed of the aborted wait
...
*                        a signal to yourself.          目标任务控制块
*
*              opt       determines the type of POST performed:
*
*                             OS_OPT_POST_NONE         No option                    没有操作    
