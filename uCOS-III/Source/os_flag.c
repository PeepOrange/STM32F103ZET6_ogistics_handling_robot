...
*
* Arguments  : p_grp          is a pointer to the event flag group to create    事件标志组指针
*
*              p_name         is the name of the event flag group       事件标志组的名字
*
*              flags          contains the initial value to store in the event flag group (typically 0).        时间标志的初始值
*
...
*
*                            OS_OPT_DEL_NO_PEND           Deletes the event flag group ONLY if no task pending              如果没有等待任务，就删除该任务标志组
*                            OS_OPT_DEL_ALWAYS            Deletes the event flag group even if tasks are waiting.           必须删除任务标志组
...
*
* Arguments  : p_grp         is a pointer to the desired event flag group.      事件标志组指针
*
*              flags         Is a bit pattern indicating which bit(s) (i.e. flags) you wish to wait for.        
*                            The bits you want are specified by setting the corresponding bits in 'flags'.
*                            e.g. if your application wants to wait for bits 0 and 1 then 'flags' would contain 0x03.   要等待的事件（标志位）的组合
*
*              timeout       is an optional timeout (in clock ticks) that your task will wait for the
*                            desired bit combination.  If you specify 0, however, your task will wait
*                            forever at the specified event flag group or, until a message arrives.                     等待超时时间（0为无期限等待）
*
...
*
*                                OS_OPT_PEND_FLAG_CLR_ALL   You will wait for ALL bits in 'flags' to be clear (0)       等待的标志位全部清零
*                                OS_OPT_PEND_FLAG_CLR_ANY   You will wait for ANY bit  in 'flags' to be clear (0)       等待的标志位任意一个清零
*                                OS_OPT_PEND_FLAG_SET_ALL   You will wait for ALL bits in 'flags' to be set   (1)       等待的标志位全部置1
*                                OS_OPT_PEND_FLAG_SET_ANY   You will wait for ANY bit  in 'flags' to be set   (1)       等待的标志位任意一个置1
*
...
*
*                                      OS_OPT_PEND_FLAG_SET_ANY + OS_OPT_PEND_FLAG_CONSUME                              把等到的值触发位取反
*
*                            You can also 'ADD' the type of pend with 'ONE' of the two option:                          
*
*                                OS_OPT_PEND_NON_BLOCKING   Task will NOT block if flags are not available              不堵塞
*                                OS_OPT_PEND_BLOCKING       Task will     block if flags are not available              堵塞直到等待到事件标志组
*
...
*                            indicates that you don't need the timestamp. 等待到的时间戳
*
...
*
*                            OS_OPT_PEND_ABORT_1          ABORT wait for a single task (HPT) waiting on the event flag  只中止这个任务标志组列表中优先级最高的等待任务
*                            OS_OPT_PEND_ABORT_ALL        ABORT wait for ALL tasks that are  waiting on the event flag  中止这个任务标志组列表所有等待任务
*                            OS_OPT_POST_NO_SCHED         Do not call the scheduler                                     不进行调度
*
...
* Arguments  : p_grp         is a pointer to the desired event flag group.
*
*              flags         If 'opt' (see below) is OS_OPT_POST_FLAG_SET, each bit that is set in 'flags' will 选择要控制的标志位
...
*
*                                OS_OPT_POST_FLAG_SET       set             对选中的位置1
*                                OS_OPT_POST_FLAG_CLR       cleared         对选中的位置0
*                                OS_OPT_POST_FLAG_CLR+OS_OPT_POST_NO_SCHED  对选中的位置0且不进行任务调度
*                                OS_OPT_POST_FLAG_CLR+OS_OPT_POST_NO_SCHED  对选中的位置1且不进行任务调度
*
