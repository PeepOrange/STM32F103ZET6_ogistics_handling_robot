...
void  OSTmrCreate (OS_TMR               *p_tmr,     //定时器控制块指针
                   ...
                   OS_TICK               dly,       //初始定时器节拍
                   OS_TICK               period,    //周期定时器重载节拍数
                   OS_OPT                opt,       //选项
                   OS_TMR_CALLBACK_PTR   p_callback,    //定时到期函数的回掉函数
                   void                 *p_callback_arg,    //传给回调函数的参数
                   OS_ERR               *p_err)     //返回错误的类型
{
    CPU_SR_ALLOC();         //进入临界段时用来保存 CPU 状态寄存器（SR）



#ifdef OS_SAFETY_CRITICAL               //如果使能安全检测
    if (p_err == (OS_ERR *)0) {         //错误类型实参为空
    ...
    if (OSSafetyCriticalStartFlag == DEF_TRUE) {    //如果是 OSSafetyCriticalStart() 后创建该定时器
       *p_err = OS_ERR_ILLEGAL_CREATE_RUN_TIME;     //错误类型为“非法创建内核对象”
        return;                                     //返回，不执行定时操作
    }
#endif

#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u                  //如果使能中断中非法调用
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* 如果该函数在中断中调用                      */
       *p_err = OS_ERR_TMR_ISR;                         //错误类型为“在中断中定时”
        return;                                     //返回，不执行定时操作
    }
#endif

#if OS_CFG_ARG_CHK_EN > 0u                              //如果使能参数检测
    if (p_tmr == (OS_TMR *)0) {                             /* 如果定时器任务控制块指针为空                                       */
       *p_err = OS_ERR_OBJ_PTR_NULL;                        //错误类型为“定时器对象为空”
        return;                                         //返回，不执行定时操作
    }

    switch (opt) {                                          //根据延迟选项 opt 分类操作
        case OS_OPT_TMR_PERIODIC:                       //如果是周期性定时
             if (period == (OS_TICK)0) {                //如果周期定时重载实参为0
    ...
    p_tmr->State          = (OS_STATE           )OS_TMR_STATE_STOPPED;     /* 初始定时器参数           */
...
#if OS_CFG_DBG_EN > 0u                                      //如果使能调试代码和变量
    OS_TmrDbgListAdd(p_tmr);                                //将该定时器添加到定时器双向调试链表
#endif
    OSTmrQty++;                                             /* 定时器个数+1             */
...
CPU_BOOLEAN  OSTmrDel (OS_TMR  *p_tmr,              //删除软件定时器
                       OS_ERR  *p_err)
{
    OS_ERR       err;
    CPU_BOOLEAN  success;   //用来暂存执行结果
    ...
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /*  如果在中断中调用                      */
       *p_err  = OS_ERR_TMR_ISR;                //错误类型 “中断中删除定时器”
        return (DEF_FALSE);
    }
#endif
    ...
    if (p_tmr == (OS_TMR *)0) {         
       *p_err = OS_ERR_TMR_INVALID;         //错误类型“定时器不存在”
        return (DEF_FALSE);
    }
#endif

#if OS_CFG_OBJ_TYPE_CHK_EN > 0u 
    if (p_tmr->Type != OS_OBJ_TYPE_TMR) {                   /* 对象类型非定时器*/
       *p_err = OS_ERR_OBJ_TYPE;        //错误类型“对象类型错误”
        return (DEF_FALSE);
    }
#endif

    OSSchedLock(&err);          //锁住调度定时器
#if OS_CFG_DBG_EN > 0u      //如果使能调试代码和变量
    OS_TmrDbgListRemove(p_tmr);     //将该定时器从双向调试链表移除
#endif
    OSTmrQty--;                                             /* 定时器个数-1                                        */

    switch (p_tmr->State) {
        case OS_TMR_STATE_RUNNING:          //正在运行
             OS_TmrUnlink(p_tmr);                           /* 从定时器轮列表移除   */
             OS_TmrClr(p_tmr);                              //复位定时器的参数
             OSSchedUnlock(&err);                           //解锁调度器
            *p_err   = OS_ERR_NONE;                         //错误类型“无错误”
             success = DEF_TRUE;
             break;

        case OS_TMR_STATE_STOPPED:                          /* 停止                      */
        case OS_TMR_STATE_COMPLETED:                        /* 完成一次定时            */
             OS_TmrClr(p_tmr);                              /* 复位定时器参数                                     */
             OSSchedUnlock(&err);                            //调度器解锁
            *p_err   = OS_ERR_NONE;
             success = DEF_TRUE;
             break;
             
        case OS_TMR_STATE_UNUSED:                           /* 未激活                                     */
             OSSchedUnlock(&err);
            *p_err   = OS_ERR_TMR_INACTIVE;                 //错误类型“定时器未激活”
             success = DEF_FALSE;
             break;

        default:                                    //定时器状态超过范围
             OSSchedUnlock(&err);   
            *p_err   = OS_ERR_TMR_INVALID_STATE;            //错误类型“定时器状态非法”
             success = DEF_FALSE;
             break;
    }
...
CPU_BOOLEAN  OSTmrStart (OS_TMR  *p_tmr,                //启动一个软件定时器
                         OS_ERR  *p_err)
{
    OS_ERR       err;
    ...
    if (p_err == (OS_ERR *)0) {                             //如果错误类型实参为空
        OS_SAFETY_CRITICAL_EXCEPTION();                     //执行安全检测异常函数
        return (DEF_FALSE);                                 //返回 DEF_FALSE ，不在执行
    }
#endif

#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u                      //如果使能中断中非法调用
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {              /* 如果该函数在中断中调用                      */
       *p_err = OS_ERR_TMR_ISR;                             //返回错误类型为“在中断中启动定时器”
        return (DEF_FALSE);                                 //返回DEF_FALSE，不在执行
    }
#endif

#if OS_CFG_ARG_CHK_EN > 0u                                  //如果使能参数检测
    if (p_tmr == (OS_TMR *)0) {                             //如果定时器控制块指针为空
       *p_err = OS_ERR_TMR_INVALID;                         //错误类型为“无效定时器”
        return (DEF_FALSE);                                 //返回DEF_FALSE，不在执行
    }
#endif

#if OS_CFG_OBJ_TYPE_CHK_EN > 0u                             //如果使能对象类型检测
    if (p_tmr->Type != OS_OBJ_TYPE_TMR) {                   /* 如果该定时器的对象类型有错                            */
       *p_err = OS_ERR_OBJ_TYPE;                            //错误类型为“对象类型错误”
        return (DEF_FALSE);                                 //返回DEF_FALSE，不继续执行
    }
#endif

    OSSchedLock(&err);                                      //锁住调度器
    switch (p_tmr->State) {                                 //根据定时器状态
        case OS_TMR_STATE_RUNNING:                          /* 如果定时器正在运行  ，则重启                                  */
             OS_TmrUnlink(p_tmr);                           /*   从定时器轮中移除该定时器                                  */
             OS_TmrLink(p_tmr, OS_OPT_LINK_DLY);            /*  再将该定时器重新插入定时器轮          */
             OSSchedUnlock(&err);                           //解锁调度器
            *p_err   = OS_ERR_NONE;                         //错误类型为“无错误”
             success = DEF_TRUE;                            //执行结果暂时未TRUE
             break;

        case OS_TMR_STATE_STOPPED:                          /* 如果定时器被停止                                       */
        case OS_TMR_STATE_COMPLETED:                        //如果定时器已经完成
             OS_TmrLink(p_tmr, OS_OPT_LINK_DLY);            /* 将该定时器重新插入定时器轮           */
             OSSchedUnlock(&err);                           //调度器解锁
            *p_err   = OS_ERR_NONE;                         //错误类型为“无错误”
             success = DEF_TRUE;                            //执行结果暂时未TRUE
             break;

        case OS_TMR_STATE_UNUSED:                           /* 如果定时器未创建                                     */
             OSSchedUnlock(&err);                           //调度器解锁
            *p_err   = OS_ERR_TMR_INACTIVE;                 //错误类型为“定时器未激活”
             success = DEF_FALSE;                           //执行结果暂为FALSE
             break;                                        
...
*
*                               OS_OPT_TMR_NONE            Do nothing special but stop the timer 单单停止
*                               OS_OPT_TMR_CALLBACK        Execute the callback function, pass it the callback argument     停止并且执行回调函数
*                                                          specified when the timer was created.
*                               OS_OPT_TMR_CALLBACK_ARG    Execute the callback function, pass it the callback argument     停止并且调用回调函数（带参数 *p_callback_arg）
                        ...
                        OS_ERR  *p_err)
{
...
#ifdef OS_SAFETY_CRITICAL                       //如果使能安全检测
    if (p_err == (OS_ERR *)0) {
        OS_SAFETY_CRITICAL_EXCEPTION();         //安全检测异常函数
        return (DEF_FALSE);
    }
#endif

#if OS_CFG_CALLED_FROM_ISR_CHK_EN > 0u                          //如果使能中断调用检测
    if (OSIntNestingCtr > (OS_NESTING_CTR)0) {                        /* 如果在中断中被调用         */
       *p_err = OS_ERR_TMR_ISR;
        return (DEF_FALSE);
    }
#endif
...
#if OS_CFG_OBJ_TYPE_CHK_EN > 0u                                 //如果使能对象类型检测
    if (p_tmr->Type != OS_OBJ_TYPE_TMR) {                             /* 如果对象类型不是软件定时器                */
       *p_err = OS_ERR_OBJ_TYPE;                                 //错误类型为“对象类型错误”        
        return (DEF_FALSE);
    }
#endif
    ...
    OSSchedLock(&err);                                          //锁住调度器
    switch (p_tmr->State) {                                    //根据定时器状态执行     
        case OS_TMR_STATE_RUNNING:                          //正在运行
             OS_TmrUnlink(p_tmr);                                     /* 从定时器轮中移除            */
            *p_err = OS_ERR_NONE;                                       //错误类型为“无错误”
             switch (opt) {
                 case OS_OPT_TMR_CALLBACK:                                  
                      p_fnct = p_tmr->CallbackPtr;                         /* 取得定时器的回调函数        */
                      if (p_fnct != (OS_TMR_CALLBACK_PTR)0) {              /* 如果回调函数存在                        */
                        (*p_fnct)((void *)p_tmr, p_tmr->CallbackPtrArg);   /* 使用创建定时器时的实参执行回调函数 */
                      } else {
                         *p_err = OS_ERR_TMR_NO_CALLBACK;                   //错误类型为“回调函数不存在”
                      }
                      break;
                 ...
                 case OS_OPT_TMR_CALLBACK_ARG:                          
                      ...
                      if (p_fnct != (OS_TMR_CALLBACK_PTR)0) {
                        (*p_fnct)((void *)p_tmr, p_callback_arg);     /* 取 p_callback_arg 作为实参执行回调函数*/
                      } else {
             ...
             OSSchedUnlock(&err);               //调度器解锁
             success = DEF_TRUE;                //执行结果暂存为成功
             break;

        case OS_TMR_STATE_COMPLETED:                                  /* 定时器已完成第一次定时 */
        case OS_TMR_STATE_STOPPED:                                    /* 定时器已经停止*/
             OSSchedUnlock(&err);                                     //调度器解锁
            *p_err   = OS_ERR_TMR_STOPPED;                              //错误类型为“定时器已经停止”
             success = DEF_TRUE;
             break;

        case OS_TMR_STATE_UNUSED:                                     /*  如果该定时器未被创建                       */
             OSSchedUnlock(&err);                                   //调度器解锁
            *p_err   = OS_ERR_TMR_INACTIVE;                         //错误类型为“定时器未激活”
             success = DEF_FALSE;
             break;
...
void  OS_TmrLink (OS_TMR  *p_tmr,               //将软件定时器插入定时器轮列表
                  OS_OPT   opt)
{
    OS_TMR_SPOKE     *p_spoke;
    OS_TMR           *p_tmr0;
    ...
    if (opt == OS_OPT_LINK_PERIODIC) {                      /* 如果定时器为再次插入                      */
        p_tmr->Match = p_tmr->Period + OSTmrTickCtr;        //匹配时间为周期重载值+软件记数
    } else {                                                //如果定时器是首次插入
        if (p_tmr->Dly == (OS_TICK)0) {                     //定时器的Dly=0
            p_tmr->Match = p_tmr->Period + OSTmrTickCtr;    //匹配时间为周期重载值+软件记数
        } else {
            p_tmr->Match = p_tmr->Dly    + OSTmrTickCtr;     //否则匹配时间为Dly+软件记数
        }
    }
    spoke  = (OS_TMR_SPOKE_IX)(p_tmr->Match % OSCfg_TmrWheelSize);      //通过Hash算法计算将该定时器插入定时器轮的具体数组
    p_spoke = &OSCfg_TmrWheel[spoke];                                   //获得该数组要插入的定时器的任务指针
    ...
    if (p_spoke->FirstPtr ==  (OS_TMR *)0) {                /* 如果列表为空                           */
        ...
        p_tmr->Remain  = p_tmr->Match                       /* 算出定时器 tmr 的剩余时间                     */
                       - OSTmrTickCtr;
        p_tmr1         = p_spoke->FirstPtr;                 /*  tmr1  为 列表的首个元素             */
        while (p_tmr1 != (OS_TMR *)0) {                    //如果tmr1非空
            p_tmr1->Remain = p_tmr1->Match                  /* 算出tmr1的剩余时间        */
                           - OSTmrTickCtr;
            if (p_tmr->Remain > p_tmr1->Remain) {           /* 如果tmr的剩余时间大于 tmr1      */
                if (p_tmr1->NextPtr  != (OS_TMR *)0) {      /* 如果tmr1后面非空   */
                    p_tmr1            = p_tmr1->NextPtr;    /* 取tmr1后面定时器为新的tmr1进行下一次循环                 */
                } else {                                    // tmr1后面为空
                    p_tmr->NextPtr    = (OS_TMR *)0;        //将 tmr 插入 tmr1 后面 
                    p_tmr->PrevPtr    =  p_tmr1;
                    p_tmr1->NextPtr   =  p_tmr;             
                    p_tmr1            = (OS_TMR *)0;        /* 结束循环                                            */
                }
            } else {                                        /* 如果tmr的剩余时间小于或等于 tmr1                 */
                if (p_tmr1->PrevPtr == (OS_TMR *)0) {       /*   如果tmr1前一个为空            */
                    p_tmr->PrevPtr    = (OS_TMR *)0;        //将tmr作为定时器轮列表第一个
                    p_tmr->NextPtr    = p_tmr1;
                    p_tmr1->PrevPtr   = p_tmr;
                    p_spoke->FirstPtr = p_tmr;
                } else {                                    /*  如果tmr1前一个非空        */
                    p_tmr0            = p_tmr1->PrevPtr;    //将tmr插入tmr1前一个
        ...
        p_spoke->NbrEntries++;                              //定时器轮列表定时器个数+1
    }   
    if (p_spoke->NbrEntriesMax < p_spoke->NbrEntries) {     /*  更新列表成员数最大值 */
...
void  OS_TmrUnlink (OS_TMR  *p_tmr)                     //将定时器从定时器轮中移除
{
    OS_TMR_SPOKE    *p_spoke;
    OS_TMR          *p_tmr1;
    OS_TMR          *p_tmr2;
    ...
    spoke   = (OS_TMR_SPOKE_IX)(p_tmr->Match % OSCfg_TmrWheelSize);     //通过Hash算法算出要插入的定时器轮的哪一个列表
    p_spoke = &OSCfg_TmrWheel[spoke];                                   //获得要插入的列表的指针

    if (p_spoke->FirstPtr == p_tmr) {                       /* 如果 tmr 是列表的首个元素    */
        p_tmr1            = (OS_TMR *)p_tmr->NextPtr;       //取tmr后一个元素为 tmr1
        p_spoke->FirstPtr = (OS_TMR *)p_tmr1;               //表首改为 tmr1
        if (p_tmr1 != (OS_TMR *)0) {                        //如果tmr1非空
            p_tmr1->PrevPtr = (OS_TMR *)0;                  //tmr1的前一个元素为空
        }
    } else {                                                //如果tmr1为空
        p_tmr1          = (OS_TMR *)p_tmr->PrevPtr;         /* tmr的前一个元素为 tmr1                */
        p_tmr2          = (OS_TMR *)p_tmr->NextPtr;         //tmr 的后一个元素为 tmr2
        p_tmr1->NextPtr = p_tmr2;                           // tmr1的后一个元素为 tmr2
        if (p_tmr2 != (OS_TMR *)0) {                        //如果tmr2非空
            p_tmr2->PrevPtr = (OS_TMR *)p_tmr1;             //tmr2的前一个元素为 tmr1
        }
    }
    p_tmr->State   = OS_TMR_STATE_STOPPED;                  //定时器状态设置为停止
    p_tmr->NextPtr = (OS_TMR *)0;                           //tmr的前后元素都清空
    p_tmr->PrevPtr = (OS_TMR *)0;
    p_spoke->NbrEntries--;                                  //列表元素成员个数-1
}
